...
def grab_headlines():
    my_url = 'http://www.michigandaily.com/section/opinion' #assinging url to the home page of the Michigan Daily
    response = requests.get(my_url) #grabbing headlines
    soup = BeautifulSoup(response.content,'html.parser')
    most_read = soup.find('h2', 'pane-title') #finding the title of the most read articles
    my_ordered_lst = most_read.find_next('ol') #making an ordered list of the most read articles
    my_headline_lst =  [] #creating empty list
    for x in my_ordered_lst.find_all('a'): #looping through the ordered list of the most read articles
        my_headline_lst.append(x.string) #adding the most read headlines to the lst 'my_headline_lst'
    return my_headline_lst #returning said list
...
def get_umsi_data():
    umsi_titles = {} #initializing empty dictionary
    page_num = 0 #initializing the page count
    beginning_of_url = 'https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastname_value=&rid=All'
    my_response = requests.get(beginning_of_url, headers={'User-Agent': 'SI_CLASS'})
    soup = BeautifulSoup(my_response.content, 'html.parser') #calling beautiful soup
    while page_num < 13: #12 total pages
        my_h2 = soup.find_all('h2')
        lst_of_names = [] #initializing empty list for names of UMSI Directory
        lst_of_name_and_titles = [] #initializing empty list for names and titles of UMSI Directory
        lst_of_titles = [] #initializing empty list for titles of UMSI Directory
        x = soup.find_all("div", { "class" : "field-item even" }) 
        for y in x:
            if y.string != None:
                lst_of_name_and_titles.append(y.string)
        for z in range(len(lst_of_name_and_titles)): #within the list of names and titles, add to the names list if odd, add to the titles list if even
            if z % 2 != 0:
                lst_of_titles.append(lst_of_name_and_titles[z])
        for h in my_h2[3:len(my_h2) - 2]:
            if '.' not in h.string: #if theres no '.' in h.string, add the name to the list of names
                lst_of_names.append(h.string)
        p = 0
        for q in lst_of_names:
            umsi_titles[q] = lst_of_titles[p]
            p += 1
        page_num += 1
        whole_url = beginning_of_url + '&amp%3Bfield_person_lastname_value=&amp%3Brid=7&page=' + str(page_num)
        html = requests.get(whole_url, headers={'User-Agent': 'SI_CLASS'})
...
def num_students(data):
    phd_students = 0  #initializing phd student count to zero
    for x in data.values(): #looping through all values in dictionary umsi_titles
        if x == 'PhD student': # if the value is equal to 'PHd student', add 1 to the count of PHD students
            phd_students += 1 
    return phd_students #return the total amount of PHD students (should be 67)
